...
class Drag(DragBase, object):
    ...
    def detail(self,url):
        try:
            ...
            #下架
            if 'sold out' in area('.soldout').text().lower() :
    ...
    def get_all_price(self,area):
        ...
        if 'Save' in area('.productoptions .saving-percent').text() :
            saveRatio = re.search(r'(\d[\d.]*)\s*%',area('.productoptions .saving-percent span').text(),re.DOTALL).groups()[0]

            listPrice = float(price) / (1-(float(saveRatio)/100))
        elif len(area('.productoptions .was-price')):
            listPrice = re.search(r'(\d[\d\.]*)',area('.productoptions .was-price').text()).groups()[0] or price

        return price,listPrice

    def get_color_img_size(self,area,pid):
        post_url = 'http://www.allsole.com/variations.json?productId={0}'.format(pid) #11243624
