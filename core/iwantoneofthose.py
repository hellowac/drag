...
class Drag(DragBase, object):
    ...
    def get_sizes(self,productId,pdata):
        try:
            ...
            if pdata :
                ...
                sizes = {}
                link = 'http://www.iwantoneofthose.com/variations.json?productId={pid}'.format(pid=productId)
                ...
                if colour and sizes_ :
                    for color in colour['options'] :
                        ...
                        for size in sizes_['options'] :
                            ...
                            obj = dict(name=size['name'],inventory=inventory,value=size['value'],sku=size['id'],price=price,oldPrice=info['rrp'])

                            sizes[color['id']].append(obj)
                            
                    if len(sizes.keys()) > 1 :
                        raise ValueError,u'iwantoneofthose find many color ,fix this bug'

                    sizes = sizes

                else :

                    sizes = [{ 'name':size['name'],'inventory':inventory,'sku':self.cfg.DEFAULT_SIZE_SKU,'id':self.cfg.DEFAULT_SIZE_SKU} for variation in pdata['variations'] if variation['variation'] == 'Size' for size in variation['options']]
    ...
    def get_all_price(self,area,pdata):
        try:
            if pdata :
                ...
                ptxt = area('div.product-details p.product-price span.price').text()
    ...
    def get_pdata(self,domain,jdata):
        try:
            ...
            # http://www.coggles.com/variations.json?productId=11135051
